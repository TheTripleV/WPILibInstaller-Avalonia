...
type DownloadStatus =
    | NotStarted
    | InProgress of ProgressInfo
    | Finished
    | Failed of ProgressInfo * string
...
module Downloader =
    ...
    let startDownloading download (progressCallback: DownloadStatus -> unit) =
        task {
            let mutable cached_progress: ProgressInfo = Unchecked.defaultof<_>
            try
                let cache (progress: ProgressInfo) =
                ...
                while isMoreToRead = true do
                    ...
                    if not isMoreToRead then
                        fileStream.Close()
                        match downloadExistsAndValid download with
                        | true -> progressCallback DownloadStatus.Finished
                        | false -> progressCallback (DownloadStatus.Failed (progress, "Downloaded file does not match expected hash"))

                    else if readCount % 100L = 0L then
                        progress
                        |> cache
                        |> DownloadStatus.InProgress
                        |> progressCallback
            ...
            with ex ->
                progressCallback
                    (DownloadStatus.Failed (cached_progress, ex.ToString()))
